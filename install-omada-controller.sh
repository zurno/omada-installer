...
if [[ $OS = *"Ubuntu"* ]]; then
    ...
    echo "deb [ signed-by=/usr/share/keyrings/mongodb-server-4.4.gpg ] http://repo.mongodb.org/apt/debian $OsVer/mongodb-org/4.4 main" | sudo tee /etc/apt/sources.list.d/mongodb-org-4.4.list
fi
...
wget -P /tmp/ $OmadaPackageUrl
echo "[+] Installing Omada Software Controller - $(basename $OmadaPackageUrl)"
